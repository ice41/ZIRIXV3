...
local obj = GetClosestObjectOfType(GetEntityCoords(ped).x, GetEntityCoords(ped).y, GetEntityCoords(ped).z, 3.0, GetHashKey(objects[1]), false, true ,true)
		...
		if distance <= 30 then
			...
			if distance <= 1.2 then
				...
				if not trabalhando and not CarroSpawnado then
					...
					if IsControlJustPressed(0,38)  then
						...
						if uniformizado then
							trabalhando = true
							notificacao("Expediente iniciado! Vá atá ao carro no estacionamento")
							
							--[[CarroSpawnado = true
							carro()]]

						else
							notificacao("Precisa estar uniformizado, visite o blip ao lado!")
						end
					
					end
				else
		    		DrawText3D(-825.55084228516,-731.86492919922,28.054931640625 + 1.5, 'PRESSIONE  ~b~E~w~  PARA ENCERRAR O EXPEDIENTE')  
							if IsControlJustPressed(0,38) or IsControlJustPressed(0,38) then
...
function carro()
    local mhash = GetHashKey('comet2')
    modelRequest('comet2')
    nveh = CreateVehicle(mhash,carro[1], carro[2], carro[3], carro[4],true,false)
    local placa = BigodeLixeiro.PegarPlaca()
