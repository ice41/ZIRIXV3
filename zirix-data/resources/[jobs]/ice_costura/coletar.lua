...
emP = Tunnel.getInterface("ice_costura")
...
local servico = false
local selecionado = 0
local CoordenadaX = 705.71
local CoordenadaY = -963.90
local CoordenadaZ = 30.39
...
-----------------------------------------------------------------------------------------------------------------------------------------
local locs = {
   --[1] = { ['x'] = 705.76,['y'] = -963.75,['z'] = 30.39 }, -- loc para TESTE -- 705.76,-963.75,30.39
	...
	while true do
		...
		if not servico then
			...
			if distance <= 30.0 then
				DrawMarker(27,CoordenadaX,CoordenadaY,CoordenadaZ -0.9,0,0,0,0,180.0,130.0,1.0,1.0,0.5,240,0,0,30,0,0,0,1)
				if distance <= 1.5 then
					drawTxt("PRESSIONE  ~b~E~w~  PARA INICIAR A RECOLHA",4,0.5,0.91,0.50,255,255,255,180)
...
Citizen.CreateThread(function()
	while true do
      Citizen.Wait(1)
      if servico then
			drawTxt("~y~PRESSIONE ~r~F7 ~w~SE DESEJA FINALIZAR A ROTA ",4,0.270,0.95,0.45,255,255,255,200)
