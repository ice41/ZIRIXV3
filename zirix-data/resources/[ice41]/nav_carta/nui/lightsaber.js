...
function sendData(name,data){
	$.post("http://nav_carta/"+name,JSON.stringify(data),function(datab){
		if (datab != "ok"){
			console.log(datab);
		}
	});
}
//Aqui começa o conteiner de perguntas//
...
*/
  ...
  $(".btnQuestion").click(function(){
      $.post('http://nav_carta/question', JSON.stringify({}));
  });
  $(".btnClose").click(function(){
      $.post('http://nav_carta/close', JSON.stringify({nbGoodAnswer}));
  });
  $(".btnKick").click(function(){
      ...
      randomQuestion = 0;
	  skip = 0;
      $.post('http://nav_carta/kick', JSON.stringify({nbGoodAnswer}));
	  nbGoodAnswer = 0;
  });
