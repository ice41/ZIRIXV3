...
--[ EVENTOS ]----------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("refinar-coca")
AddEventHandler("refinar-coca", function()
    local source = source
            ...
            if vRP.tryGetInventoryItem(user_id,"folhas-coca",5) then
                if vRP.tryGetInventoryItem(user_id,"calcio-po",1) then
                    ...
                    SetTimeout(10000,function()
                        ...
                        TriggerClientEvent("Notify",source,"sucesso","Produziu <b>1x Compósito de Média Qualidade</b>.")
                    end)
            
                else
                    TriggerClientEvent("Notify",source,"negado","Não tem <b>Cálcio em Pó</b> na mochila.")
                    vRP.giveInventoryItem(user_id,"folhas-coca",5)
                    vRP.giveInventoryItem(user_id,"acido-sulfurico",1)
                end
            else
                TriggerClientEvent("Notify",source,"negado","Não tem <b>Folhas de Coca</b> na mochila.")
                vRP.giveInventoryItem(user_id,"acido-sulfurico",1)
            end
        else
            TriggerClientEvent("Notify",source,"negado","Não tem <b>Ácido Sulfúrico</b> na mochila.")
        end
    else
            ...
            if vRP.tryGetInventoryItem(user_id,"folhas-coca",10) then
                if vRP.tryGetInventoryItem(user_id,"calcio-po",1) then
                    ...
                    SetTimeout(10000,function()
                        ...
                        TriggerClientEvent("Notify",source,"sucesso","Produziu <b>1x Compósito de Alta Qualidade</b>.")
                    end)
